...
class Interactions extends React.Component {
  constructor(props) {
    ...
    const query = queryString.parse(props.location.search);
      ServerAPI.getNeighbors(query.ID).then(res=>{ 
  ...
  parse(data){
    ...
    if(!nodeMap.has(splitLine[0])){
      ...
      network.nodes.push({data:{bbox:{h:15,w:15,x:7.5,y:7.5},class: "macromolecule",clonemarker:false,id: splitLine[0],label: splitLine[0],parsedMetadata:[],stateVariables:[],unitsOfInformation:[]}});
    }
    if(!nodeMap.has(splitLine[2])){
      nodeMap.set(splitLine[2],true);
      network.nodes.push({data:{bbox:{h:15,w:15,x:7.5,y:7.5},class: "macromolecule",clonemarker:false,id: splitLine[2],label: splitLine[2],parsedMetadata:[],stateVariables:[],unitsOfInformation:[]}});
    }
